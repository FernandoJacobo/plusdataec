    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res
                .status(500)
                .json({
                    message: "Error al consultar rangos de honorarios",
                    error: error.message,
                });
        }
    }
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (!nombre || !correo || !celular || !mensaje) {
                res
                    .status(500)
                    .json({ error: true, message: "Faltan campos obligatorios" });
                return;
            }
            ...
            if (!process.env.SMTP_SERVER || !process.env.SMTP_PORT) {
                res
                    ...
                    .json({
                        error: true,
                        message: "Credenciales del servidor SMTP no configuradas.",
                    });
                return;
            }
            ...
            let htmlTemplate = fs.readFileSync(
                path.join(__dirname, "../../public/email-templates/message.html"),
                "utf8"
            );
            ...
            try {
                ...
                res
                    ...
                    .json({
                        error: false,
                        message:
                            "Gracias por contactarnos, Enseguida nos pondremos en contacto contigo.",
                    });
            } catch (error) {
                console.error("Error al enviar correo:", error);
                res
                    .status(500)
                    .json({ error: true, message: "Error al enviar correo" });
            }
        } catch (error: any) {
            res
                .status(500)
...
router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            db.query(
                "INSERT INTO mensajes (nombre, correo, celular, mensaje) VALUES (?, ?, ?, ?)",
